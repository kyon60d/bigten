...
function ccd.DrawInit()
    lazy_init()
end

function ccd.DrawFree()
    dp_initialized = false
end

function ccd.DrawColor4f(r,g,b,a)
    dp_color[1] = r
    dp_color[2] = g
    dp_color[3] = b
    dp_color[4] = a
...
function ccd.DrawPoints(points,numOfPoint)
    if not lazy_init() then
        return
    end
...
function ccd.DrawLine(origin,destination)
    if not lazy_init() then
        return
    end
...
function ccd.DrawPoly(points,numOfPoints,closePolygon)
    if not lazy_init() then
        return
    end
...
function ccd.DrawSolidPoly(points,numOfPoints,color)
    if not lazy_init() then
        return
    end
...
function ccd.DrawRect(origin,destination)
    ccd.DrawLine(CCPoint:__call(origin.x, origin.y), CCPoint:__call(destination.x, origin.y))
    ccd.DrawLine(CCPoint:__call(destination.x, origin.y), CCPoint:__call(destination.x, destination.y))
    ccd.DrawLine(CCPoint:__call(destination.x, destination.y), CCPoint:__call(origin.x, destination.y))
    ccd.DrawLine(CCPoint:__call(origin.x, destination.y), CCPoint:__call(origin.x, origin.y))
end

function ccd.DrawSolidRect( origin,destination,color )
    local vertices = { origin, CCPoint:__call(destination.x, origin.y) , destination, CCPoint:__call(origin.x, destination.y) }
    ccd.DrawSolidPoly(vertices,4,color)
end

function ccd.DrawCircleScale( center, radius, angle, segments,drawLineToCenter,scaleX,scaleY)
    if not lazy_init() then
        return
    end
...
function ccd.DrawCircle(center, radius, angle, segments, drawLineToCenter)
    ccd.DrawCircleScale(center, radius, angle, segments, drawLineToCenter, 1.0, 1.0)
end

function ccd.DrawSolidCircle(center, radius, angle, segments,scaleX,scaleY)
    if not lazy_init() then
        return
    end
...
function ccd.DrawQuadBezier(origin, control, destination, segments)
    if not lazy_init() then
        return
    end
...
function ccd.DrawCubicBezier(origin, control1, control2, destination, segments)
    if not lazy_init then
        return
    end
